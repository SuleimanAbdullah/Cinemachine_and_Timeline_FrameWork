        ...
        void FixedUpdate()
        {
            ...
            if (_trackRotation == true) //check track rotation bool
            {
                ...
                else if (_missileType == Missile_Launcher.MissileType.Homing) //if missle is homing
                {
                    ...
                    Vector3 turnAmount = Vector3.Cross(transform.forward, direction); //using cross product, we multiply our forward vector of the rocket by the direction vector,
                                                                                      //to create a perpendular vector, which specifies the turn amount
